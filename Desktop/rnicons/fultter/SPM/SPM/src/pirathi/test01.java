...
public class test01 extends javax.swing.JFrame {
    ...
    private void loadRoom() {
        ...
        try {
            String query = "SELECT * FROM roomforsession";
...
//            rooms.add("Select");
    ...
    private void loadSession() {      
              ...
              String zz=romComboBox.getSelectedItem().toString();
            PreparedStatement stmt = null;
            ResultSet rs = null;

            try {
                String query = "SELECT * FROM Sessions INNER JOIN SessionLecturers ON Sessions.Id=SessionLecturers.id WHERE Sessions.groupid='"+ gp +"'";
                stmt = conn.prepareStatement(query);
                rs = stmt.executeQuery();
...
//                sessions.add("Select");          
                while (rs.next()) {
                sessions.add(rs.getString("lecturer")+" "+rs.getString("subject") + " " + rs.getString("subcod") + " "
                        + rs.getString("tag") + " " + rs.getString("Id") + " "
                        + rs.getString("count") + " " + rs.getString("duration")+" "+ zz + "\n");              
                }                      
...
//                String query3 = "select time_slot from time_slot where batch=(select batch from workingdays where batch='" + timetableList.get(0) + "')";
    ...
    private void initComponents() {
        ...
        jLabel17.setText("Genarate Time Table");
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        PreparedStatement stmt,s1 = null;
        ResultSet rs1 = null;
        
      

        try {
             String qq="select * from  timetable where timeSlot ='"+timeslot+"' and day='"+day+"' and session='"+session+"'";
        s1 = conn.prepareStatement(qq);

            rs1 = s1.executeQuery(); 
            if(rs1.next()==true){
JOptionPane.showMessageDialog(this, "Already Allocated !!!");            } 
            else{
